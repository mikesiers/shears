...
class test_auxilliary_functions(unittest.TestCase):
  ...
  def test_estimated_errors(self):
    ...
    self.assertEqual(round(estimated_error(0, 6), 3), 1.238)
    self.assertEqual(round(estimated_error(0, 9), 3), 1.284)
    self.assertEqual(round(estimated_error(0, 1), 3), 0.750)
